...
int triad_aligned(int len, double xx, double * a, double * b, double * c)
{
	...
	for(i=0; i< len; i++) a[i] = b[i] + xx*c[i];
	return bytes;
}

int triad_aligned_restrict(int len, double xx, double *restrict a, double *restrict b, double *restrict c)
{
...
{
	int i, bytes=24;
	for(i=0; i< len; i++) a[i] = triad_base(b[i] ,c[i], xx);
	return bytes;
}

int triad_restrict(int len, double xx, double *restrict a, double *restrict b, double *restrict c)
{
	int i, bytes=24;
	for(i=0; i< len; i++) a[i] = b[i] + xx*c[i];
	return bytes;
...
int triad_wrier10(int len, double xx, double * a, double * b, double * c)
{
	int i, bytes=24;
